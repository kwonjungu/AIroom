...
- **데이터**: `patrol-log.json` — 오브젝트 `{config, days}` (배열 아님 → ARRAY_COLLECTIONS 미등록).
  ...
  days = `{ "YYYY-MM-DD": {date, name, staffId, rounds:{회차idx:{items:[0미점검|1이상없음|2이상있음], memo, at, sign:{img,at,name}}}, confirm:{img,name,at}} }`. 서명 img는 240×90 PNG dataURL. **순찰자 서명은 회차(시간)별** — 구 스키마의 day 레벨 `sign`은 읽기 호환만 유지(plBatchTargets).
