...
- **데이터**: `patrol-log.json` — 오브젝트 `{config, days}` (배열 아님 → ARRAY_COLLECTIONS 미등록).
  ...
  days = `{ "YYYY-MM-DD": {date, name, staffId, rounds:{회차idx:{items:[0미점검|1이상없음|2이상있음], memo, at}}, sign:{img,at,name}, confirm:{img,name,at}} }`. 서명 img는 240×90 PNG dataURL.
...
- **서명**: 포인터 이벤트 캔버스(마우스+터치). 순찰자 서명 = 본인/관리자만. 확인자 서명 = 관리모드, 직위+성명 입력(교장/교감 칩 자동 노출), **일괄 확인 서명** = 순찰 기록 있고 확인 전인 날 전체에 한 번에 적용.
