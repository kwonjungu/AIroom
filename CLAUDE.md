...
- **서버 라우트**: `PATCH /api/patrol-log/days/:date` (하루 단위 저장, 분산 락), `POST /api/patrol-log/setup` (config+days 교체 — 기존 기록 삭제). GET/POST는 DATA_ROUTES 자동. 공휴일은 기존 `/api/winter-schedule/holidays` 재사용.
