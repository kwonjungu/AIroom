...
function submitAll(r, stage) { for (const v of r.reviewers.filter(v => v.stages.includes(stage))) { d.saveEvaluation(r, v.id, stage, rows(r, stage)); d.submitEvaluation(r, v.id, stage); } return r; }
...
test('unsubmitted reviewers never reduce an average to zero; duplicate names retain identity', () => {
    ...
    d.saveEvaluation(r, v.id, 'document', rows(r, 'document')); d.submitEvaluation(r, v.id, 'document');
    ...
    const v2 = r.reviewers[1]; d.saveEvaluation(r, v2.id, 'document', rows(r, 'document', .5)); d.submitEvaluation(r, v2.id, 'document');
...
test('null and zero differ; finite scores, caps, bonus threshold and reasons enforced', () => {
    const r = provision(d.createRecruitment(input({ allowBonus: true }))), v = r.reviewers[0]; const draft = rows(r, 'document'); draft[0].scores.education = null;
...
module.exports = { input, rows, submitAll };
