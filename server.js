...
// Mail: authenticated same-origin proxy, page gate, and shared account registry.
...
app.use('/api/mail/provider', mailAuth, express.json({ type: 'application/merge-patch+json', limit: '16kb' }), require('./lib/mail-proxy'));
...
// 계정 등록 — 브라우저가 mail.tm에 계정을 만든 뒤 발급 주소를 명부에 기록
app.post('/api/mail/accounts/:name', mailAuth, async (req, res) => {
    ...
    try {
        const store = (await readData('mail-accounts.json')) || {};
        if (!store[name] || store[name].address !== address) {
            store[name] = { address, createdAt: new Date().toISOString() };
