...
<p class="small muted" style="margin-top:18px">순위는 동점 시 공동순위입니다. 설정은 생성 후 고정되며 면접대상자는 서류 평가 완료 후 관리자가 확정합니다.</p><div class="actions"><button class="primary">채용 생성</button><button type="button" class="secondary" data-action="cancel-new">취소</button></div></form></section>`;
    ...
    function render() {
        ...
        if (admin && state.config) {
            const google = state.config.google || {};
            $('#app aside').insertAdjacentHTML('beforeend', `<section class="panel small" style="padding:16px"><h3>Google 계정 연결</h3><p class="muted">${google.connected ? '인증 정보 저장됨 · 접근 확인 필요' : google.configured ? 'OAuth 설정 완료 · 계정 승인 필요' : 'OAuth 클라이언트 JSON 설정 필요'}</p><div class="actions"><button class="secondary" data-action="google-connect" ${google.configured ? '' : 'disabled'}>${google.connected ? '다시 인증' : 'Google 계정 연결'}</button>${google.connected ? '<button class="secondary" data-action="google-check">Drive 접근 확인</button>' : ''}</div><p class="small muted" style="margin-top:12px">평가 진행에는 필요 없습니다. 결과 문서를 Drive에 보관하려는 경우에만 연결하세요.</p></section>`);
        }
        state.dirty = false;
        setupSignature();
        syncTotals();
    ...
    function exportsPanel(r) {
        return `<section class="panel"><h2>확정 결과 문서</h2><p class="muted">원본 시트의 평가 항목을 반영한 통계표·위원별 채점표·청렴서약서를 만듭니다.</p>${!r.snapshot ? '<p class="notice">모든 평가를 제출하고 통계·결과 탭에서 결과를 확정하세요.</p>' : r.actor.role !== 'admin' ? '<p class="notice">전체 결과 문서 다운로드는 관리자가 진행합니다.</p>' : `<div class="download-grid"><button class="primary" data-action="download" data-format="html"><strong>HTML → PDF</strong><small>인쇄용 서식 열기</small></button><button class="secondary" data-action="download" data-format="xlsx"><strong>Excel 통계표</strong><small>위원별 점수·집계·평가 상세</small></button><button class="secondary" data-action="download" data-format="zip"><strong>전체 ZIP</strong><small>HTML · Excel · 확정 데이터</small></button></div><p class="notice" style="margin-top:20px">HTML 인쇄 화면의 ‘인쇄 / PDF로 저장’을 누르고 PDF로 저장을 선택하세요.</p>${r.archive ? `<div class="notice"><strong>Drive 보관됨</strong> · ${stamp(r.archive.archivedAt)}<br>${esc(r.archive.files.join(', '))}<br>자동 삭제 예정: ${stamp(r.archive.purgeAfter)}<br><a href="${esc(r.archive.folderUrl)}" target="_blank" rel="noopener noreferrer">보관 폴더 열기 ↗</a></div>` : ''}<div class="actions"><button class="secondary" data-action="archive">${r.archive ? 'Drive 보관 다시 올리기' : 'Drive에 보관 (7일 뒤 자동 삭제)'}</button></div><p class="small muted" style="margin-top:10px">Drive 보관은 Google 계정 연결이 되어 있어야 합니다. 삭제는 별도 예약 작업이 아니라 누군가 보관을 실행할 때 함께 정리됩니다.</p>`}${r.actor.role === 'admin' && state.config ? `<a class="secondary" href="${esc(state.config.folderUrl)}" target="_blank" rel="noopener noreferrer">Drive 저장 폴더 열기 ↗</a>` : ''}</section>`;
    }
    ...
    $('#app').addEventListener('submit', e => {
        ...
        run(async () => {
            ...
            if (form.id === 'create-form') {
                const f = new FormData(form); const reviewers = Array.from(form.querySelectorAll('.reviewer')).map(row => ({ name: row.querySelector('[name=reviewer-name]').value, position: row.querySelector('[name=reviewer-position]').value, email: row.querySelector('[name=reviewer-email]').value, stages: ['document', 'interview'].filter(s => row.querySelector(`[name=${s}]`).checked) }));
    ...
    $('#app').addEventListener('click', e => { const button = e.target.closest('[data-action]'); if (!button) return; const action = button.dataset.action;
        run(async () => {
            ...
            if (action === 'fill-example') { const form = $('#create-form'); if (!form) return; if (state.dirty && !confirm('현재 입력을 예시로 바꿀까요?')) return; Object.entries({ school: '예시초등학교', title: '2026 기초학력 협력강사 채용 (예시)', field: '기초학력 협력강사', shortlistLimit: '2', candidateCount: '3' }).forEach(([k, v]) => { form.elements[k].value = v; });
                syncCandidates(['예시지원자 가', '예시지원자 나', '예시지원자 다']); $('#reviewer-rows').innerHTML = reviewerRow({ name: '예시위원 가', email: 'reviewer1@example.com' }) + reviewerRow({ name: '예시위원 나', email: 'reviewer2@example.com' }); state.dirty = true; }
            if (action === 'google-connect') { if (!canLeave()) return; const data = await api('/google/start', 'POST', {}); location.href = data.authorizationUrl; }
            if (action === 'google-check') { const data = await api('/google/check', 'POST', {}); notify(`${data.email} · ${data.folderName}: ${data.message}`); }
