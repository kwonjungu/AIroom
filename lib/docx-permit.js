...
// 연수기간:27% | 구분:11% | 일수:7% | 연수내용:21% | 연수장소:17% | 비고:17%
const PERIOD_COL_WIDTHS = [2603, 1061, 675, 2024, 1639, 1637]; // 합 9639

function buildPeriodTable(fortyOnePeriods, workPeriods, sum41, sumWork) {
    const rows = [];
    ...
    rows.push(rowOf(
        ['연수기간', '근무/41조', '일수', '연  수   내  용', '연 수 장 소', '비고(연락처)'],
        W, { bold: true, fill: 'D6D6D6', heightTwip: ROW_H }
    ));
    const periodsForOutputWork = workPeriods.length > 0 ? workPeriods : [{ range: '', days: '' }];
    for (const pr of periodsForOutputWork) {
        rows.push(rowOf(
            [pr.range || '', '', pr.days === '' ? '' : String(pr.days), pr.content || '', pr.place || '', pr.note || ''],
            W, { heightTwip: ROW_H }
        ));
...
async function generatePermitDocx(data) {
    ...
    const sumWork = workPeriods.reduce((s, x) => s + (Number(x.days) || 0), 0);
    ...
    const doc = new Document({
        ...
        sections: [{
            ...
            children: [
                ...
                p(''),
                buildPeriodTable(fortyOnePeriods, workPeriods, sum41, sumWork),
                p(''),
                p('휴가 중 교원 근무상황일람표', { align: AlignmentType.CENTER, bold: true, size: 26 }),
                p(''),
