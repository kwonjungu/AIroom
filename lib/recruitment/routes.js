...
function createRouter({ validateSession, redis, serverless = false, directory = path.join(__dirname, '../../data/recruitment'), store = createStore({ redis, serverless, directory }) }) {
    ...
    async function admin(req, res, next) { try { const session = await validateSession(req.get('X-Auth-Token')); domain.assert(session?.role === 'admin', '백암이 관리자 로그인이 필요합니다.', session ? 403 : 401); req.actor = { role: 'admin', id: 'admin' }; next(); } catch (e) { next(e); } }
    async function access(req, res, next) {
        try {
            const r = await store.get(req.params.id);
            const session = await validateSession(req.get('X-Auth-Token'));
            if (session?.role === 'admin') req.actor = { role: 'admin', id: 'admin' };
            else {
                const token = req.get('X-Recruitment-Token') || ''; domain.assert(/^[a-f0-9]{64}$/.test(token), '위원 초대 링크 또는 관리자 로그인이 필요합니다.', 401);
