...
function signatureBlock(reviewer, s) {
    ...
    return `날짜: ${on ? escape(on) : '______년 ____월 ____일'}<br>직위: ${escape(reviewer.position)}<br>성명: ${escape(reviewer.name)}　${mark}`
        + (on ? `<p class="sign-stamp">위 서명은 ${escape(s.school)} 채용 관리 시스템에서 본인 확인 링크로 접속한 위원이 ${escape(new Date(reviewer.pledge.signedAt).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }))}에 직접 입력한 전자서명입니다.</p>` : '');
}
    ...
    for (const reviewer of s.reviewers) {
        ...
        for (const stage of reviewer.stages) {
            ...
            for (let start = 0; start < ev.rows.length; start += 5) {
                ...
                const rows = RUBRICS[stage].map(item => [item.label, item.max, ...group.map(row => row.attendance === 'absent' ? '불참' : row.scores[item.id])]);
                if (stage === 'document') rows.push(['가점', s.rules.allowBonus ? 5 : '미사용', ...group.map(row => row.bonus)]);
                rows.push(['합계', stage === 'document' && s.rules.allowBonus ? 55 : 50, ...group.map(rowTotal)]);
...
async function workbook(r) {
    ...
    sheet('평가위원 및 서약', ['위원', '직위', '담당 전형', '청렴서약서 서명', '서명 일시'],
        s.reviewers.map(v => [v.name, v.position, v.stages.map(label).join(' · '), v.pledge?.signedAt ? '서명 완료 (전자서명)' : '미서명',
    ...
    for (const ev of Object.values(s.evaluations)) for (const row of ev.rows) {
        ...
        for (const item of RUBRICS[ev.stage]) details.push([label(ev.stage), c.code, c.name, v.name, v.position, item.label, item.max, row.attendance === 'absent' ? '불참' : row.scores[item.id], '', row.note, ev.submittedAt]);
...
async function exportResult(r, format) {
    ...
    const zip = new JSZip(); zip.file('평가통계_확정본.xlsx', xlsx); zip.file('채점표_서약서_인쇄용.html', printHtml(r)); zip.file('확정데이터.json', JSON.stringify(r.snapshot, null, 2)); zip.file('읽어주세요.txt', 'HTML 파일을 브라우저에서 열고 인쇄 / PDF로 저장 버튼을 누르세요.\nGoogle 미연결 상태에서 생성한 로컬 결과 묶음입니다.\n서약서 문구는 기관 확인 후 사용하며 자필 서명은 별도입니다.');
