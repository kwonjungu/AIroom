...
function printHtml(r) {
    ...
    // 공문서로 쓰는 출력물이라 시스템 이름·내부 버전·생성 시각은 싣지 않는다.
    const header = title => `<header><p>${escape(s.school)} · ${escape(s.field)}</p><h1>${escape(title)}</h1><p>${escape(s.title)}</p></header>`;
    const footer = '';
    const sections = [];
    for (const stage of ['document', 'interview']) {
        const stat = statistics(s, stage);
        sections.push(`<section class="page landscape">${header(stage === 'document' ? '서류심사 집계표' : '면접·최종 합산 통계표')}<div class="rule">최종 순위 기준: <strong>${escape(basis)}</strong> · 동점은 공동순위 (1, 2, 2, 4)<br>전형 평균은 배정 위원 전원 제출 후 계산, 소수 첫째 자리 반올림. 합산은 표시된 두 전형 평균의 합.<br>서류 기본 ${total(s, 'document')}점${s.rules.allowBonus ? ' + 확인된 가점 최대 5점' : ' · 가점 미사용'} / 면접 ${total(s, 'interview')}점. 불참은 0점과 구분하고 순위에서 제외.</div>${table(stat.headers, stat.rows)}${stage === 'document' && s.guidance ? `<div class="rule"><strong>심사 참고자료</strong><br>${escape(s.guidance).replace(/\n/g, '<br>')}</div>` : ''}${rubric(s, stage).some(item => item.guide) ? `<div class="rule"><strong>채점 기준</strong><br>${rubric(s, stage).filter(item => item.guide).map(item => `${escape(item.label)} (${item.max}점): ${escape(item.guide).replace(/\n/g, '<br>　')}`).join('<br>')}</div>` : ''}<p class="note">${stage === 'document' ? `면접대상자 선정 사유: ${escape(s.shortlistReason)}` : '이 표는 평가 순위이며, 동점자의 최종 선발은 기관의 확정된 기준에 따라 별도로 결정합니다.'}</p>${footer}</section>`);
    }
    for (const reviewer of s.reviewers) {
        sections.push(`<section class="page pledge">${header('청렴서약서 (평가위원용)')}<p>본인은 ${escape(s.school)}의 ${escape(s.field)} 채용 심사를 실시함에 있어 다음 사항을 준수할 것을 서약합니다.</p><ol><li>객관적이고 공정한 평가를 위하여 관련 업체 또는 개인위탁강사와의 이해관계를 확인하고, 이해충돌이 있는 경우 담당자에게 알리겠습니다.</li><li>평가와 관련하여 금품·향응·편의를 수수하거나 제공받지 않으며, 이러한 상황이 발생하면 담당 부서에 통보하겠습니다.</li><li>업무상 취득한 비밀을 준수하고 보안 관련 규정과 지침을 성실히 수행하겠습니다.</li><li>평가와 관련하여 알게 된 업무상 비밀을 타인에게 누설하지 않겠습니다.</li></ol><p class="note">원본 서약서의 핵심 항목을 반영한 출력 초안입니다. 기관의 확정 문구를 확인한 뒤 사용하세요.</p><div class="signature">${signatureBlock(reviewer)}<br><br>${escape(s.school)}장 귀하</div>${footer}</section>`);
        for (const stage of reviewer.stages) {
            const ev = s.evaluations[`${stage}:${reviewer.id}`];
            // A4 가로 지면을 채우도록 10명 단위. 5명 단위는 최대 규모에서 채점표만 20세트가 됐다.
            const GROUP = 10;
            for (let start = 0; start < ev.rows.length; start += GROUP) {
                const group = ev.rows.slice(start, start + GROUP);
                ...
                sections.push(`<section class="page">${header(`${label(stage)} 채점표`)}<p>채점자 직위: ${escape(reviewer.position)}　성명: ${escape(reviewer.name)}　${reviewer.pledge?.image ? `<img class="sign-inline" src="${reviewer.pledge.image}" alt="${escape(reviewer.name)} 서명">` : '(서명 또는 인)'}</p><p>전형일: ${escape(stage === 'document' ? s.documentDate : s.interviewDate)} · 대상 ${start + 1}~${start + group.length}명</p>${table(headers, rows)}${stage === 'document' && s.rules.allowBonus ? '<p class="note">가점은 별도 근거 확인 후 적용합니다.</p>' : ''}${footer}</section>`);
                // 평가 메모 별지는 싣지 않는다(2026-09-17 지시). 근거는 Excel 상세 시트에 남는다.
            }
        }
    }
    ...
    *{box-sizing:border-box}body{font-family:'Malgun Gothic','Apple SD Gothic Neo',sans-serif;color:#17232b;background:#eceff1;margin:0;font-size:10.5pt;line-height:1.45}.toolbar{padding:18px;background:#fff;position:sticky;top:0;border-bottom:1px solid #ccc}.toolbar button{font:inherit;background:#0066cc;color:#fff;border:0;padding:9px 18px;border-radius:6px}.page{background:#fff;max-width:1120px;margin:24px auto;padding:36px;break-after:page}.page:last-child{break-after:auto}header{border-bottom:2px solid #17232b;margin-bottom:12px;padding-bottom:8px}h1{font-size:17pt;margin:4px 0}p{margin:5px 0}.rule,.note{font-size:9pt;color:#354650;margin:8px 0}table{width:100%;border-collapse:collapse;font-size:9.5pt;table-layout:fixed}th,td{border:1px solid #aab4bd;padding:4px 5px;text-align:center;overflow-wrap:anywhere;white-space:pre-wrap}thead{display:table-header-group}th{background:#f0f3f5}tr{break-inside:avoid}footer{font-size:8pt;color:#64727c;margin-top:12px;border-top:1px solid #ccc;padding-top:6px}.sign-image{display:inline-block;vertical-align:middle;height:52px;max-width:230px;object-fit:contain}.sign-inline{vertical-align:middle;height:34px;max-width:150px;object-fit:contain}.sign-blank{color:#5a6a74}.pledge{max-width:850px}.pledge li{margin:24px 0}.signature{text-align:right;margin:40px 0}.toolbar span{margin-left:15px;font-size:14px}@page{size:A4 landscape;margin:12mm}@media print{body{background:white}.toolbar{display:none}.page{padding:0;margin:0;max-width:none;min-height:0}.pledge{padding:0 30mm}a{color:inherit}}@media(max-width:700px){.page{padding:16px;margin:12px 0}table{font-size:9pt}.toolbar span{display:block;margin:10px 0 0}}
