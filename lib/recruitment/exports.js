    ...
    for (const reviewer of s.reviewers) {
        ...
        for (const stage of reviewer.stages) {
            ...
            for (let start = 0; start < ev.rows.length; start += GROUP) {
                ...
                sections.push(`<section class="page">${header(`${label(stage)} 채점표`)}<p>채점자 직위: ${escape(reviewer.position)}　성명: ${escape(reviewer.name)}　${reviewer.pledge?.image ? `<img class="sign-inline" src="${reviewer.pledge.image}" alt="${escape(reviewer.name)} 서명">` : '(서명 또는 인)'}</p><p>전형일: ${escape(stage === 'document' ? s.documentDate : s.interviewDate)} · 대상 ${start + 1}~${start + group.length}명</p>${table(headers, rows)}${start === 0 && rubric(s, stage).some(item => item.guide) ? `<div class="rule"><strong>평가 준거</strong><br>${rubric(s, stage).filter(item => item.guide).map(item => `${escape(item.label)} (${item.max}점): ${escape(item.guide).replace(/\n/g, '<br>　')}`).join('<br>')}</div>` : ''}${stage === 'document' && s.rules.allowBonus ? '<p class="note">가점은 별도 근거 확인 후 적용합니다.</p>' : ''}${footer}</section>`);
