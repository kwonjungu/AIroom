...
// 문서는 구역 3개다(한/글의 '구역 나누기'): 0=표지(세로), 1=집계·채점표(가로), 2=청렴서약서(세로).
// 구역마다 Contents/sectionN.xml 한 개 + 그 안 첫 문단의 secPr 이 자기 용지 설정을 들고 있다.
...
// ── 단위 ──────────────────────────────────────────────────────────────────────
// 1mm = 283.465 HWPUNIT. A4 세로(210×297mm) / 가로(297×210mm), 여백 15mm.
const MM = 283.465;
...
// landscape="WIDELY" + 59528×84188. 실기 판별(2026-09-17, A/B 샘플): 한/글은 NARROWLY
// 플래그로 용지를 '회전'하므로 pagePr 의 width/height 는 회전 전(세로) 값 그대로 둬야 한다.
// NARROWLY + 치수 교환은 두 번 회전이 되어 도로 세로가 된다. 본문 배치 폭만 가로 기준으로 쓴다.
// groupSize: 채점표 한 장에 넣을 지원자 수. 본문 폭이 넓어지는 가로 구역에서만 늘린다.
const PORTRAIT = { landscape: 'WIDELY', paperWidth: 59528, paperHeight: 84188, width: 59528, margin: 4251, groupSize: 5 };
const LANDSCAPE = { landscape: 'NARROWLY', paperWidth: 59528, paperHeight: 84188, width: 84188, margin: 4251, groupSize: 8 };
// 표 폭은 구역마다 다르다 — 세로 51026, 가로 75686.
const bodyWidth = page => page.width - page.margin * 2;
...
function headerXml(sectionCount) {
    ...
    const charPrs = [
        // 크기 위계(2026-09-17 지시: 기본 11pt): 본문·표 11 / 절 제목 14 / 표지 제목 20 / 주석 9.5.
        // 표도 11pt 로 올린다 — 가로 구역(본문 폭 75686)이라 13열 통계표도 줄바꿈 없이 들어간다.
        charPrXml(CHAR.body, { size: 1100 }),
        charPrXml(CHAR.bold, { size: 1100, bold: true }),
        charPrXml(CHAR.title, { size: 2000, bold: true }),
        charPrXml(CHAR.heading, { size: 1400, bold: true }),
        charPrXml(CHAR.cell, { size: 1100 }),
        charPrXml(CHAR.cellHead, { size: 1100, bold: true }),
        charPrXml(CHAR.note, { size: 950, color: '#4A4A4A' }),
...
function table(body, matrix, weights) {
    ...
    const widths = columnWidths(body.width, colCnt, weights);
    ...
    const tbl = `<hp:tbl id="${body.id()}" zOrder="0" numberingType="TABLE" textWrap="TOP_AND_BOTTOM" textFlow="BOTH_SIDES" lock="0" dropcapstyle="None" pageBreak="TABLE" repeatHeader="1" rowCnt="${matrix.length}" colCnt="${colCnt}" cellSpacing="0" borderFillIDRef="${BORDER.cell}" noAdjust="0">`
        + `<hp:sz width="${body.width}" widthRelTo="ABSOLUTE" height="${CELL_MIN_HEIGHT * matrix.length}" heightRelTo="ABSOLUTE" protect="0"/>`
...
function coverSection(body, s) {
    ...
    blank(body);
    // 순위 기준·확정 시각은 내부 산정 정보라 공문서 표지에 넣지 않는다 (2026-09-17 지시).
    const info = [
    ...
    blank(body);
    if (s.guidance) {
        text(body, '심사 참고자료', { charPr: CHAR.heading, paraPr: PARA.left });
        for (const line of String(s.guidance).split(/\r?\n/)) text(body, line, { charPr: CHAR.body, paraPr: PARA.justify });
        blank(body);
    }
}
...
function scoreSheets(body, s, reviewer) {
    for (const stage of reviewer.stages) {
        ...
        // 지원자를 몇 명씩 끊어 표를 나눈다. 더 넣으면 가로 폭이 모자라 글자가 뭉갠다.
        // 한도는 구역의 용지 방향이 정한다(세로 5명 / 가로 8명).
        const size = body.page.groupSize;
        for (let start = 0; start < ev.rows.length; start += size) {
            const group = ev.rows.slice(start, start + size);
            ...
            // 어떤 기준으로 채점했는지 채점표에 함께 남긴다. 표 안에 넣으면 폭이 모자라 문단으로 싣는다.
            const guided = s.rubrics[stage].filter(item => item.guide);
            if (guided.length && start === 0) {
                text(body, '평가 준거', { charPr: CHAR.bold });
                for (const item of guided) {
                    const lines = String(item.guide).split(/\r?\n/).filter(v => v.trim());
                    text(body, `${item.label} (${item.max}점): ${lines[0]}`, { charPr: CHAR.note, paraPr: PARA.justify });
                    for (const extra of lines.slice(1)) text(body, `　${extra}`, { charPr: CHAR.note, paraPr: PARA.justify });
                }
                blank(body);
            }
            ...
            // 평가 메모는 인쇄본에 싣지 않는다(2026-09-17 지시). 가점·불참 근거는 Excel 상세 시트에 남는다.
        }
    }
}
function pledgeSection(body, s, reviewer) {
    // 구역의 첫 문단이면 쪽 나눔을 걸지 않는다 — 구역 나누기 자체가 이미 쪽을 넘긴다.
    // (그 문단은 secPr 을 품는 자리라, 여기에 pageBreak 를 더하면 빈 쪽이 하나 생긴다.)
    text(body, '청렴서약서', { charPr: CHAR.title, paraPr: PARA.center, pageBreak: body.parts.length > 0 });
    blank(body);
    text(body, `본인은 ${s.school}의 ${s.field} 채용 심사를 실시함에 있어 다음 사항을 준수할 것을 서약합니다.`, { charPr: CHAR.body, paraPr: PARA.justify });
    blank(body);
...
function sectionXml(body) {
    ...
    const secPr = '<hp:secPr id="" textDirection="HORIZONTAL" spaceColumns="1134" tabStop="8000" tabStopVal="4000" tabStopUnit="HWPUNIT" outlineShapeIDRef="1" memoShapeIDRef="0" textVerticalWidthHead="0" masterPageCnt="0">'
        ...
        // 용지 설정은 구역마다 다르다 — 이것이 한/글의 '구역 나누기 + 용지 방향'과 같은 효과를 낸다.
        // width/height 는 방향과 무관하게 세로 기준 고정값. 회전은 landscape 플래그가 담당한다.
        + `<hp:pagePr landscape="${page.landscape}" width="${page.paperWidth}" height="${page.paperHeight}" gutterType="LEFT_ONLY"><hp:margin header="0" footer="0" gutter="0" left="${page.margin}" right="${page.margin}" top="${page.margin}" bottom="${page.margin}"/></hp:pagePr>`
...
async function buildHwpx(r) {
    ...
    const doc = createDoc();
    // 구역 0(세로): 표지 — 제목 + 기본정보표.
    const cover = createBody(doc, PORTRAIT);
    coverSection(cover, s);
    // 구역 1(가로): 열이 많은 표들. 세로 폭으로는 지원자가 늘수록 글자가 뭉갠다.
    const tables = createBody(doc, LANDSCAPE);
    statSection(tables, s, 'document', '1. 서류심사 집계표', [
    ...
    statSection(tables, s, 'interview', '2. 면접·최종 합산 통계표', [
    ...
    for (const reviewer of s.reviewers) scoreSheets(tables, s, reviewer);
    // 구역 2(세로): 청렴서약서 — 글만 있는 문서라 세로가 맞다.
    const pledges = createBody(doc, PORTRAIT);
    for (const reviewer of s.reviewers) pledgeSection(pledges, s, reviewer);
    const sections = [cover, tables, pledges];
