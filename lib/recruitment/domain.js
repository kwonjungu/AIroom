...
function publicView(r, actor) { const copy = structuredClone(r); copy.reviewers.forEach(v => { delete v.inviteHash; delete v.inviteExpiresAt; }); return { ...copy, actor, rubrics: RUBRICS, documentResults: totals(r, 'document'), finalResults: r.snapshot?.results || finalResults(r) }; }
module.exports = { DomainError, assert, text, createRecruitment, saveEvaluation, submitEvaluation, reopen, selectShortlist, finalize, totals, finalResults, publicView, stageCandidates, RUBRICS, audit };
