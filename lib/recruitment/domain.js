...
// guide 는 "무엇을 보고 몇 점을 주는가"(평가 준거)다. 위원마다 기준이 다르면 점수가 흔들리므로
// 채점 화면과 채점표 출력에 항목과 함께 붙여 둔다.
const DEFAULT_RUBRICS = {
    document: [
        { label: '학력', max: 6, guide: '관련 전공 학위·이수 학점을 확인합니다. 상위 학위 또는 직무 연관 전공일수록 높게 봅니다.' },
        { label: '자격증', max: 6, guide: '직무 관련 자격증의 종류와 등급을 확인합니다. 제출된 사본으로 확인되는 것만 인정합니다.' },
        { label: '경력', max: 6, guide: '동일·유사 직무 경력 연수를 확인합니다. 경력증명서로 확인되는 기간만 계산합니다.' },
        { label: '지속 근무', max: 6, guide: '한 기관에서의 근속 기간과 중도 이직 빈도를 봅니다. 채용 기간을 채울 수 있는지 판단합니다.' },
        { label: '자기소개서', max: 26, guide: '지원 동기, 직무 이해, 학생 지도 계획의 구체성과 실현 가능성을 봅니다. 분량이 아니라 내용으로 판단합니다.' }
    ],
    interview: [
        { label: '인성', max: 10, guide: '책임감과 협력 태도를 봅니다. 답변의 일관성과 사례의 구체성으로 판단합니다.' },
        { label: '교직관', max: 10, guide: '교육에 대한 관점과 학생을 대하는 자세를 봅니다. 추상적 신념보다 실제 경험을 근거로 봅니다.' },
        { label: '소양', max: 10, guide: '의사 표현의 명료함과 기본 소양을 봅니다. 질문 의도를 정확히 파악하는지 확인합니다.' },
        { label: '수업 이해', max: 10, guide: '담당할 수업의 목표와 방법을 이해하고 있는지 봅니다. 구체적 지도 방안을 제시하는지 확인합니다.' },
        { label: '학생 이해', max: 10, guide: '학습 부진·정서 어려움이 있는 학생에 대한 이해와 대응 방안을 봅니다.' }
    ]
};
...
function parseRubrics(input) {
    ...
    for (const stage of STAGES) {
        ...
        rubrics[stage] = items.map((item, index) => {
            ...
            const guide = item.guide === undefined || item.guide === null || String(item.guide).trim() === '' ? '' : text(item.guide, `${name} ${item.label} 평가 준거`, 500);
            return { id: `c${index + 1}`, label: text(item.label, `${name} 항목 이름`, 40), max: half(item.max, `${name} ${item.label} 배점`, 0.5, 100), guide };
        });
...
function createRecruitment(input, actor = 'admin') {
    ...
    assert(Array.isArray(input.reviewers) && input.reviewers.length >= 1 && input.reviewers.length <= 5, '평가위원은 1~5명입니다.');
    const reviewers = input.reviewers.map(v => {
        assert(v && typeof v === 'object', '위원 정보를 확인하세요.');
        // 위원은 초대 링크로 접속한다. 메일을 자동 발송하지 않으므로 주소는 받지 않는다.
        // 옛 데이터와 외부 호출 호환을 위해 값이 오면 형식만 확인하고 없으면 null 로 둔다.
        const email = v.email === undefined || v.email === null || v.email === '' ? null : text(v.email, '이메일', 254).toLowerCase();
        assert(email === null || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email), '위원 이메일 형식을 확인하세요.');
    ...
    const emails = reviewers.map(v => v.email).filter(Boolean);
    assert(new Set(emails).size === emails.length, '위원 이메일은 중복될 수 없습니다. 같은 위원에게 두 전형을 배정하세요.');
    assert(new Set(reviewers.map(v => `${v.name}|${v.position}`)).size === reviewers.length, '같은 이름·직위의 위원이 둘 있습니다. 한 줄에서 두 전형을 모두 선택하세요.');
    for (const stage of STAGES) assert(reviewers.filter(v => v.stages.includes(stage)).length >= 1 && reviewers.filter(v => v.stages.includes(stage)).length <= 5, '각 전형에 1~5명의 위원을 배정하세요.');
    ...
    // 항목별 준거로 담기 어려운 공통 지침(자격 요건, 결격 사유, 배점 해석 등)을 위원에게 함께 보여준다.
    const guidance = input.guidance === undefined || input.guidance === null || String(input.guidance).trim() === ''
        ? '' : text(input.guidance, '심사 참고자료', 3000);
    ...
    const r = { id: randomUUID(), version: 1, createdAt: new Date().toISOString(), school, title, field, documentDate, interviewDate, candidates, reviewers, rubrics, guidance,
