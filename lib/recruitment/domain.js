...
const RUBRICS = {
    document: [{ id: 'education', label: '학력', max: 6 }, { id: 'certificate', label: '자격증', max: 6 }, { id: 'experience', label: '경력', max: 6 }, { id: 'continuity', label: '지속 근무', max: 6 }, { id: 'statement', label: '자기소개서', max: 26 }],
    interview: [{ id: 'personality', label: '인성', max: 10 }, { id: 'teaching', label: '교직관', max: 10 }, { id: 'communication', label: '소양', max: 10 }, { id: 'lesson', label: '수업 이해', max: 10 }, { id: 'student', label: '학생 이해', max: 10 }]
};
const STAGES = Object.keys(RUBRICS);
...
function createRecruitment(input, actor = 'admin') {
    ...
    const r = { id: randomUUID(), version: 1, createdAt: new Date().toISOString(), school, title, field, documentDate, interviewDate, candidates, reviewers,
        rules: { rankingBasis: input.rankingBasis, shortlistLimit: input.shortlistLimit, allowBonus: input.allowBonus, bonusThreshold: 20, tiePolicy: 'competition-rank-manual-review', precision: 1 },
...
function validateRows(r, stage, rows, complete) {
    ...
    return rows.map(row => {
        ...
        const scores = {};
        for (const criterion of RUBRICS[stage]) {
...
function finalize(r) {
    ...
    r.snapshot = { version: 1, finalizedAt: new Date().toISOString(), rules: structuredClone(r.rules), results, documentResults: totals(r, 'document'), evaluations: structuredClone(r.evaluations), candidates: structuredClone(r.candidates), reviewers: r.reviewers.map(({ inviteHash, inviteExpiresAt, ...v }) => v), school: r.school, title: r.title, field: r.field, documentDate: r.documentDate, interviewDate: r.interviewDate, shortlist: [...r.shortlist], shortlistReason: r.shortlistReason };
    r.status = 'finalized'; audit(r, 'admin', '결과 확정', '확정본 v1'); return r;
}
function publicView(r, actor) { const copy = structuredClone(r); copy.reviewers.forEach(v => { delete v.inviteHash; delete v.inviteExpiresAt;
    // 서명은 본인 필적이다. 위원에게는 남의 서명 이미지를 넘기지 않고 서명 여부만 알린다.
    if (v.pledge && actor?.role === 'reviewer' && v.id !== actor.id) delete v.pledge.image; }); return { ...copy, actor, rubrics: RUBRICS, documentResults: totals(r, 'document'), finalResults: r.snapshot?.results || finalResults(r) }; }
module.exports = { DomainError, assert, text, createRecruitment, saveEvaluation, submitEvaluation, signPledge, signed, reopen, selectShortlist, finalize, totals, finalResults, publicView, stageCandidates, RUBRICS, audit };
