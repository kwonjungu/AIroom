...
export function createVibeRouter(express, deps) {
  ...
  router.get('/health', (req, res) => {
    res.json({ ok: !kvUnsafe, contractVersion: CONTRACT_VERSION, storage: deps.redis ? 'redis' : 'memory', modules: ['session', 'projects', 'progress', 'jobs', 'assets'], assetProvider: env.VIBE_ASSET_PROVIDER || 'none' });
  });
  ...
  const api = createVibeApi({ express, kv: createKvFromEnv({ redis: deps.redis }), validateStaffSession: deps.validateStaffSession, env });
  router.use(api);
  // WP6: 에셋 생성·조회 (공급자 기본 none — env로만 켠다)
  router.use(createAssetRoutes({ express, services: api.services, env }));
  // WP5: generation — 통합 시 연결
