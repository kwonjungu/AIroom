...
export function createAssetPipeline(deps) {
  ...
  async function runQueued(env, workerId) {
    ...
    return callProvider(e, workerId, () => withTimeout(e, signal => provider.generate(requestOf(e), { signal })));
  }
  ...
  // AbortSignal.timeout은 Node 22에서 이벤트 루프를 붙잡지 않아, 다른 할 일이 없으면 응답 없는 호출을 끊기 전에
  // 프로세스가 끝난다. 일반 타이머로 걸고 호출이 끝나면 반드시 해제한다.
  function withTimeout(env, fn) {
    const left = Math.max(1, Math.min(L.stepTimeoutMs, Date.parse(env.job.deadlineAt) - now()));
    const ctl = new AbortController();
    const t = setTimeout(() => ctl.abort(), left);
    return Promise.resolve().then(() => fn(ctl.signal)).finally(() => clearTimeout(t));
  }
  ...
  async function runGenerating(env, workerId) {
    ...
    if (env.priv.providerJobId && provider?.poll) {
      return callProvider(env, workerId, () => withTimeout(env, signal => provider.poll(env.priv.providerJobId, { signal })));
    }
