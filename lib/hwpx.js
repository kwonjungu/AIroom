...
//
// ⚠️ 텍스트를 실제로 바꿀 때는 문단의 <hp:linesegarray>를 lineseg 1개로 정규화해야 한다.
// 템플릿에 두 줄짜리 셀(lineseg 2개)이 있는데 텍스트만 비우거나 바꾸면 본문과 줄 배치
// 정보가 어긋나고, 한글 2024가 이를 "문서가 손상되었거나 변조되었을 가능성"으로 판정해
// [문서 보안 설정]을 낮음으로 바꿔야만 열리는 문서가 된다 (실제 겪은 버그).
function setTcText(tcXml, value, charPrOverride) {
    const re = /<hp:run charPrIDRef="(\d+)"(?:\/>|>[\s\S]*?<\/hp:run>)/;
    const cur = tcXml.match(/<hp:t>([^<]*)<\/hp:t>/);
    const curText = cur ? cur[1] : '';
    const newText = value == null ? '' : String(value);
    const changed = (curText !== newText) || (charPrOverride != null);

    let out = tcXml.replace(re, (m, cp) => {
        const cpr = charPrOverride || cp;
        if (newText === '') return `<hp:run charPrIDRef="${cpr}"/>`;
        return `<hp:run charPrIDRef="${cpr}"><hp:t>${xmlEscape(newText)}</hp:t></hp:run>`;
    });

    if (changed && curText !== newText) {
        out = out.replace(/<hp:linesegarray>([\s\S]*?)<\/hp:linesegarray>/, (m, inner) => {
            const first = inner.match(/<hp:lineseg\b[^>]*\/>/);
            return first ? `<hp:linesegarray>${first[0]}</hp:linesegarray>` : m;
        });
    }
    return out;
}
...
// 직위 표기 정규화 — 허가원 서명부에는 교장/교감/교사만 사용
// (staff.json의 '교무부장', '3-친절' 같은 담당 표기는 전부 '교사'로)
function normalizePosition(pos) {
    const p = String(pos || '');
    if (p.includes('교장')) return '교장';
    if (p.includes('교감')) return '교감';
    return '교사';
}

// "휴가중 교원 근무상황일람표" 제목과 달력 표 사이 빈 줄 1개.
// 사용자가 한글에서 수정한 샘플(권준구 (4).hwpx)의 빈 문단 XML을 그대로 사용.
const CALENDAR_TITLE_GAP_PARA =
    '<hp:p id="0" paraPrIDRef="21" styleIDRef="0" pageBreak="0" columnBreak="0" merged="0">'
    + '<hp:run charPrIDRef="26"/>'
    + '<hp:linesegarray><hp:lineseg textpos="0" vertpos="20252" vertsize="1600" textheight="1600" baseline="1360" spacing="-80" horzpos="0" horzsize="51024" flags="393216"/></hp:linesegarray>'
    + '</hp:p>';
// 달력 표를 담는 문단의 시작 (tbl id로 유일하게 식별)
const CALENDAR_TABLE_PARA_OPEN =
    '<hp:p id="2147483648" paraPrIDRef="21" styleIDRef="0" pageBreak="0" columnBreak="0" merged="0"><hp:run charPrIDRef="27"><hp:tbl id="1135405105"';
...
async function generatePermit(data) {
    ...
    const replacements = [
        ...
        // 제목과 달력 표 사이 빈 줄 1개 삽입
        [CALENDAR_TABLE_PARA_OPEN, CALENDAR_TITLE_GAP_PARA + CALENDAR_TABLE_PARA_OPEN],
    ];
